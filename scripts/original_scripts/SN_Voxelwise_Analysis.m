...
root_folder = ['C:\Users\...\'];%%end rootfolder with a fileseparator (\ or /)
...
NM_filestring = 's1_psc_wr...nii';% choose a filestring to select the fully preprocessed (s1_psc_wr) NM image here
topslicepath = [root_folder 'top_slice.mat'];
load(topslicepath)
scan_key_path = [root_folder 'scan_key.mat'];
load(scan_key_path);
...
if filter_subjects==1
    filter = [scan_key(:,3)==1]-[scan_key(:,20)==1];
else
...
exc_vox = 2; %this is a switch re: whether (=1 or =2) or not (=0) to exclude these voxels. If =1, the user can select the threshold of exclusion in the lines below. If =2 an automated threshold will be set to exclude the lowest and highest 1% of values based on the distribution of all SN voxels in all subjects combined.
LowVox = -8; %this excludes voxels with CNR value below -8% (if the switch exc_vox=1). These likely should be modified in a study specific manner based on the distribution of all voxels in all subjects
HiVox = 40; %this excludes voxels with CNR above 40%. 


%%no more user input needed below this line%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


VmaskFull = spm_read_vols(spm_vol([root_folder filesep 'SN_Mask.nii']));%This is where you load the 'full' SN mask file
...
for s=1:length(scan_key(:,1))
    ...
    if scan_key(s,2)==1
        NMscanname = dir([root_folder num2str(Subs{s}) NM_subfolder filesep NM_filestring]);
        NMscannames = [root_folder num2str(Subs{s}) NM_subfolder filesep NMscanname(1,1).name];
        v = spm_vol(NMscannames);
    elseif scan_key(s,2)==2
        NMscanname = dir([root_folder2 num2str(Subs{s}) NM_subfolder2 filesep NM_filestring]);
        NMscannames = [root_folder2 num2str(Subs{s}) NM_subfolder2 filesep NMscanname(1,1).name];
        v = spm_vol(NMscannames);
    end
