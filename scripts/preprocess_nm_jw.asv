...
%JW: script modified to re-run preprocessing for social doors nm_jw January 2021
...
if intensity_norm==1
    Vmask = spm_read_vols(spm_vol([root_folder 'CC_mask.nii'])); %this is where a user loads the reference region mask
...
if make_top_slice==1
    VmaskSN = spm_read_vols(spm_vol([root_folder 'full_SN_mask.nii']));%This is where to load SN mask file (manually traced in MNI space)
